...
describe('Knowledge Base Migration Tests', () => {
  ...
  beforeAll(() => {
    ...
    migrationSql = fs.readFileSync(
      path.join(__dirname, '../../migrations/20240201_create_knowledge_base_tables.sql'),
      'utf8'
    )
  })
  ...
  describe('RLS Policy Consistency Tests', () => {
    it('should use consistent role checks in RLS policies', () => {
      const roleChecks = migrationSql.match(/role IN \('admin', '[^']+'\)/g) || []
      expect(roleChecks.length).toBeGreaterThan(0)
      
      // All role checks should use consistent role names
      roleChecks.forEach(check => {
        expect(check).toMatch(/role IN \('admin', '(?:team_lead|agent)'(?:, '(?:team_lead|agent)')?\)/)
      })
    })

    it('should use consistent policy structure', () => {
      const policies = migrationSql.match(/CREATE POLICY "[^"]+"/g) || []
      expect(policies.length).toBeGreaterThan(0)
      
      // All policies should specify TO authenticated
      const authenticatedPolicies = migrationSql.match(/TO authenticated/g) || []
      expect(authenticatedPolicies.length).toBe(policies.length)
      
      // All policies should have either USING or WITH CHECK
      const policyChecks = migrationSql.match(/(?:USING|WITH CHECK) \(/g) || []
